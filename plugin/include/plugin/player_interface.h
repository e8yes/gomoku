...
struct MatchResultInfo {
  ...
  std::string termination_reason;  // "five_in_a_row", "resignation", "draw", etc.
};
...
class IPlayer {
 public:
  ...
  // Returns the chosen action ID (0-224 for placement, 225-229 for Swap2 choices).
  virtual int InquireAction(const BoardState& board) = 0;

  // Informs the engine or player of the action actually applied to the board state
  // after an inquiry or the opponent's move.
  ...
  // Optional: returns current estimated win rate (e.g. in [-1.0, 1.0] or [0.0, 1.0]).
  // Can be called concurrently while an action inquiry is still in-flight.
  virtual std::optional<float> GetEstimatedWinRate() const { return std::nullopt; }

  // Optional: returns current policy (action probability distribution over all 230 actions).
  // Can be called concurrently while an action inquiry is still in-flight.
