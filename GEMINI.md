...
- **VCF solver**: Correctness is the priority. Test against known VCF win/loss positions. It returns the first found winning sequence for the current player if one exists, otherwise it returns an empty vector.
- **VCT Solver**: Implement a bounded Threat-Space Search / Proof-Number VCT solver invoked **only at the MCTS search root** (with a strict node/time budget and Zobrist transposition table). Invoking at the root executes only once per turn, eliminating deep tactical dilution and securing proven forced wins without degrading parallel leaf-evaluation throughput.
- **Input Representation (Rejection of Handcrafted SHL Planes)**: Handcrafted heuristic feature planes (e.g., Suggestive Hotspot Links / SHL) are intentionally excluded from the ResNet input tensor. The 7-block SE-ResNet's receptive field ($31\times 31$) already spans the entire $15\times 15$ board and learns exact non-linear pattern combinatorics natively. Handcrafted continuous planes would increase dataset sample size by $+125\%$ (from 716 B to 1,616 B) and introduce CPU preprocessing bottlenecks during GPU-batched self-play.
